...
def crc_calculation_manual(block):
    polynomial = BitArray('0b10001000000100001')
    poly_size = 17
    crc_init = b'\x00\x00'

    block_crc = BitArray(block + crc_init)
    for i in range(len(block) * 8):
        bit = block_crc[0]
        if bit == 1:
            block_crc.overwrite(block_crc[:poly_size] ^ polynomial, 0)
        block_crc = block_crc[1:]

    return block_crc.uint
...
if __name__ == '__main__':
    ...
    # sbf_file = open("gps_gal_raw.sbf", 'br')
    sbf_file = open("trim_gps_gal_raw.sbf", 'br')
    ...
    while header := sbf_file.read(8):
        if header[:2] == SYNC:
            ...
            if length % 4 != 0:
                print(f"Length not multiple of 4")
                sbf_file.seek(file_pos := file_pos + 1)
                continue
